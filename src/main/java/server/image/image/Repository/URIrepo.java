...
public interface URIrepo extends JpaRepository <URI, Long> {
    @Query(value = "Select uid, filename, reg_date, contentType FROM URI WHERE UID = ?1", nativeQuery = true)
    Optional<URI> findByUid(@Param("_uid") Long uid);

    @Query(value = "select uid from URI", nativeQuery = true)
    public Optional<URI> showTables();

    @Query(value = "select uid, filename, reg_date, contentType from URI", nativeQuery = true)
    public Optional<URI> selectAllData();


    @Query(value = "Select uid, filename, reg_date, contentType FROM URI WHERE UID = 100000", nativeQuery = true)
